...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # bell_on_complete: play terminal bell (\a) only after the interactive
        # input queue is drained, so queued multi-turn batches do not fire early.
    ...
    def _emit_cli_queue_drained_hook(
        ...
        *,
        ...
        Core only detects that the CLI finished a successful turn and both
        user-input queues are empty. Plugins own concrete side effects such as
        completion voice, desktop notifications, or telemetry.
        """
        try:
